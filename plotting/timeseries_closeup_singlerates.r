...
for (scenarioId in c('03b')) {
	plotList <- list()
	for (algo in c('BIC', 'C3')) {
		...
		rate <- rate[rate$ClientId=='Client0' | rate$ClientId=='Client1' | rate$ClientId=='Client2',]			

		trimLeft <- 9000
		trimRight <- 14000
		per_client.rate <- rate[rate$Timestamp > trimLeft,]
		per_client.rate <- per_client.rate[per_client.rate$Timestamp < trimRight,]

		plotList[[listPos]] <- ggplot(per_client.rate) +
	...
	png(paste(scenarioId, "_closeup_singlerates.png", sep=""), height=3072, width=3072)
